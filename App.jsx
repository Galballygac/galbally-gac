...
const PITCHES = [
  { id:"main",       name:"Pearse Park",    irishName:"Páirc an Phiarsaigh", icon:"⚽", desc:"Full size · 15-a-side",       halfable:true,  mapsUrl:"https://maps.app.goo.gl/TyPtsghScR16xWW19" },
  { id:"hurson",     name:"Hurson Park",    irishName:"Páirc Uí Ursain",     icon:"🏃", desc:"Training pitch",               halfable:true,  mapsUrl:"https://maps.app.goo.gl/aDr1v1QNh4AkReC79" },
  { id:"parcnagael", name:"Parc Na Gael",   irishName:"Páirc na nGael",      icon:"🏃", desc:"Training pitch",               halfable:true,  mapsUrl:"https://maps.app.goo.gl/nAFtNV1tackyVg3x5" },
  { id:"juvenile",   name:"Parc Na Nog",    irishName:"Páirc na nÓg",        icon:"👦", desc:"Youth pitch",                  halfable:false, mapsUrl:"https://maps.app.goo.gl/5ehtP5RoEgWrjs897" },
  { id:"gym",        name:"Gym",            irishName:"An Halla",             icon:"🏋️", desc:"Bookable hall",                halfable:false, mapsUrl:"https://maps.app.goo.gl/7cHTtWrHdTReejnD8" },
  { id:"committee",  name:"Committee Room", irishName:"Seomra Coiste",        icon:"🪑", desc:"Meeting room · max 20 people", halfable:false, mapsUrl:"https://maps.app.goo.gl/TyPtsghScR16xWW19" },
];
...
function rowsToMap(rows){
  const m={};
  rows.forEach(r=>{ m[r.slot_key]={ coachName:r.coach_name, group:r.group_name, duration:r.duration, time:r.start_time, half:r.half, continuation:r.continuation||false }; });
  return m;
}
...
export default function App() {
  ...
  function handleTeamInput(val){
    ...
    if(val.trim().length>0){ setSuggestions(TEAMS.filter(t=>t.toLowerCase().includes(val.toLowerCase()))); }
    else { setSuggestions([]); }
  }
  ...
  async function handleBook(){
    if(!selPitch||!selDate||!selTime||!teamName) return;
    setSaving(true);
    ...
    for(const date of dates){
      ...
      if(allFree){
        for(let i=0;i<slots.length;i++){
          const t=slots[i];
          await upsertBooking({ slot_key:slotKey(selPitch,selHalf,dk,t), pitch_id:selPitch, half:selHalf, date_key:dk, time_slot:t, coach_id:teamName.toLowerCase().replace(/\s/g,"_"), coach_name:teamName, group_name:teamName, duration:selDur, start_time:selTime, continuation:i>0 });
        }
    ...
    if(bookedDates.length>0) setCalPrompt({dates:bookedDates,pitchName:pn,half:selHalf,time:selTime,dur:selDur,coachName:teamName});
      ...
      <div style={S.loginCard} className="fade-in">
        ...
        <div style={{position:"relative"}}>
          <div style={S.formGroup}>
            ...
            <input style={S.input} placeholder="e.g. Senior Mens, U16 Girls…" value={teamInput} onChange={e=>handleTeamInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&confirmTeam()} autoFocus/>
          </div>
          {suggestions.length>0&&(
            <div style={S.suggestions}>
              {suggestions.map(s=>(<div key={s} style={S.suggestion} onClick={()=>confirmTeam(s)}>{s}</div>))}
            </div>
          )}
        </div>
                ...
                <div key={di} style={{...S.schedDay,...(isPast?{opacity:0.45}:{}),...(isToday?S.schedDayToday:{})}}>
                  ...
                  {dayB.length===0?<div style={S.noB}>Free</div>:dayB.map(b=>{ const isC=b.pitch.id==="committee"; return (
                    <div key={b.key} style={{...S.chip,borderLeftColor:isC?"#ea580c":"#1d4ed8",background:isC?"#fff7ed":"#eff6ff"}}>
                      ...
                      <button style={S.cancelXBtn} onClick={()=>setConfirmModal(b.key)}>✕</button>
                    </div>
                  ); })}
                </div>
              );
            ...
            {upcoming.map(b=>{ const p=PITCHES.find(x=>x.id===b.pitchId); const isC=p?.id==="committee"; return (
              <div key={b.key} style={{...S.bRow,borderLeftColor:isC?"#ea580c":"#1d4ed8"}}>
                ...
                <div style={S.bRowRight}>
                  ...
                  <button style={S.cancelFullBtn} onClick={()=>setConfirmModal(b.key)}>Cancel</button>
                </div>
              </div>
            ); })}
          </div>
  ...
  return (
    <div style={S.root}>
      ...
      <header style={S.header}>
        <div style={S.headerInner}>
          ...
          <nav style={S.nav}>
            ...
            {teamName&&<><button style={{...S.navBtn,...(view==="pitches"?S.navBtnActive:{})}} onClick={()=>setView("pitches")}>Pitches & Rooms</button><button style={{...S.navBtn,...(view==="book"?S.navBtnActive:{})}} onClick={()=>setView("book")}>Book</button></>}
            {teamName?<button style={S.coachPill} onClick={changeTeam}>👤 {teamName} ✎</button>:<button style={S.signInBtn} onClick={()=>{setTeamInput("");setSuggestions([]);setShowTeamModal(true);}}>+ Book a Pitch</button>}
          </nav>
        </div>
      </header>
      <main style={S.main}>
        ...
        {view==="pitches"&&teamName&&(
          <div className="fade-in">
            <div style={S.pageHero}><div><h1 style={S.h1}>Facilities</h1><p style={S.heroIrish}>Páirc an Phiarsaigh · Galbally Pearses GAC · Est. 1949</p><p style={S.sub}>Booking as <strong>{teamName}</strong> — select a pitch or room</p></div></div>
            <div style={S.facilitySection}>
              <div style={S.facilityHeading}>⚽ Pitches & Training Areas</div>
              <div style={S.pitchGrid}>{PITCHES.filter(p=>p.id!=="committee").map((p,i)=>(<div key={p.id} className="pitch-card" style={S.pitchCard}><div style={{...S.pitchBand,background:"linear-gradient(135deg,#1e3a8a,#1d4ed8)"}}><span style={{fontSize:30}}>{p.icon}</span><div style={{fontSize:30,fontWeight:700,color:"rgba(255,255,255,0.2)"}}>{String(i+1).padStart(2,"00")}</div></div><div style={S.pitchBody}><h2 style={S.pitchName}>{p.irishName}</h2><p style={S.pitchIrish}>{p.name}</p><p style={S.pitchDesc}>{p.desc}</p>{p.halfable&&<div style={S.halfTag}>⬛ Half-pitch available</div>}<div style={S.pitchActions}><button style={{...S.bookNowBtn,background:"linear-gradient(135deg,#1e3a8a,#1d4ed8)"}} onClick={()=>{setSelPitch(p.id);setSelHalf("full");setView("book");}}>Book →</button><button style={S.mapBtn} onClick={()=>window.open(p.mapsUrl,"_blank")}>📍 Map</button></div></div></div>))}</div>
            </div>
            <div style={S.facilitySection}>
              <div style={S.facilityHeading}>🪑 Meeting Rooms</div>
              <div style={S.pitchGrid}>{PITCHES.filter(p=>p.id==="committee").map(p=>(<div key={p.id} className="pitch-card" style={{...S.pitchCard,maxWidth:320}}><div style={{...S.pitchBand,background:"linear-gradient(135deg,#7c2d12,#ea580c)"}}><span style={{fontSize:30}}>{p.icon}</span><div style={{fontSize:30,fontWeight:700,color:"rgba(255,255,255,0.2)"}}>CM</div></div><div style={S.pitchBody}><h2 style={{...S.pitchName,color:"#9a3412"}}>{p.irishName}</h2><p style={S.pitchIrish}>{p.name}</p><p style={S.pitchDesc}>{p.desc}</p><div style={{...S.halfTag,color:"#9a3412",background:"#fff7ed",border:"1px solid #fed7aa"}}>🪑 Whole room only</div><div style={S.pitchActions}><button style={{...S.bookNowBtn,background:"linear-gradient(135deg,#7c2d12,#ea580c)"}} onClick={()=>{setSelPitch(p.id);setSelHalf("full");setView("book");}}>Book Room →</button></div></div></div>))}</div>
            </div>
          </div>
        )}
        {view==="book"&&teamName&&(
          <div className="fade-in">
            <div style={S.pageTitle}><h1 style={S.h1t}>Make a Booking</h1><p style={{color:"#475569",fontSize:13}}>Booking as <strong style={{color:"#1d4ed8"}}>{teamName}</strong></p></div>
            <section style={S.section}>
            ...
            {selTime&&(<section style={S.section}>
              ...
              {blockBook&&(<div style={S.blockOptions}><div style={S.blockWeeksLabel}>Weeks:</div><div style={S.blockWeeksRow}>{[1,2,3,4].map(n=>(<button key={n} style={{...S.weekNumBtn,...(blockWeeks===n?S.weekNumBtnActive:{})}} onClick={()=>setBlockWeeks(n)}>{n}w</button>))}</div><div style={S.previewList}>{getBlockDates(selDate,blockWeeks).map((d,i)=>{ const dk=getDateKey(d); const conflict=!getSlots(selPitch,selHalf,dk,selTime,selDur).every(t=>!isBooked(selPitch,selHalf,dk,t)); return (<div key={i} style={{...S.previewItem,...(conflict?S.previewConflict:S.previewOk)}}><span>{conflict?"⚠️":"✅"}</span><span>{d.toLocaleDateString("en-IE",{weekday:"short",day:"numeric",month:"short"})}</span><span style={{marginLeft:"auto",fontSize:11}}>{selTime} · {selDur}</span>{conflict&&<span style={S.conflictBadge}>Conflict</span>}</div>); })}</div></div>)}
              <div style={S.summary}><div style={S.summaryHdr}><div><div style={S.summaryClub}>Galbally Pearses GAC</div><div style={S.summarySubtitle}>{blockBook?`Block · ${blockWeeks}w`:"Confirmation"}</div></div></div><div style={S.summaryGrid}>{[["Team",teamName],["Facility",pitch?.name],["Section",selPitch==="committee"?"Whole Room":selHalf==="full"?"Full Pitch":selHalf==="north"?"North Half":"South Half"],["Date",selDate?.toLocaleDateString("en-IE",{weekday:"short",day:"numeric",month:"long"})],["Time",selTime],["Duration",selDur]].map(([k,v])=>(<div key={k} style={S.summaryItem}><span style={S.summaryKey}>{k}</span><span style={S.summaryVal}>{v}</span></div>))}</div></div>
      ...
      {confirmModal&&(<div style={S.overlay}><div style={S.modal} className="fade-in"><div style={S.modalTitle}>Cancel Booking?</div><p style={S.modalText}>This will free the slot for other teams.</p><div style={S.modalBtns}><button style={S.modalKeep} onClick={()=>setConfirmModal(null)}>Keep It</button><button style={{...S.modalDel,opacity:saving?0.6:1}} onClick={confirmCancel} disabled={saving}>{saving?"Cancelling…":"Yes, Cancel"}</button></div></div></div>)}
