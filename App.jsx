...
export default function App() {
  ...
  function isAvailable(pid,half,dk,t,dur){ return getSlots(pid,half,dk,t,dur).every(s=>!isBooked(pid,half,dk,s)); }
  function getBlockDates(start,weeks){ return Array.from({length:weeks},(_,i)=>{ const d=new Date(start); d.setDate(d.getDate()+i*7); return d; }); }
  ...
  async function handleBook(){
    ...
    for(const date of dates){
      ...
      if(slots.every(t=>!isBooked(selPitch,selHalf,dk,t))){
        for(let i=0;i<slots.length;i++){
          await upsertBooking({ slot_key:slotKey(selPitch,selHalf,dk,slots[i]), pitch_id:selPitch, half:selHalf, date_key:dk, time_slot:slots[i], coach_id:teamName.toLowerCase().replace(/\s/g,"_"), coach_name:teamName, group_name:teamName, duration:selDur, start_time:selTime, continuation:i>0 });
        }
  ...
  async function confirmCancel(){
    ...
    for(const t of getSlots(pid,half,dk,b.time,b.duration||"1 hour")) await deleteBooking(slotKey(pid,half,dk,t));
  ...
  function buildICSEnd(date,time,dur){ const [h,m]=time.split(":").map(Number); const d=new Date(date); d.setHours(h,m+({["30 mins"]:30,["1 hour"]:60,["1.5 hours"]:90,["2 hours"]:120}[dur]||60),0,0); return d.toISOString().replace(/[-:]/g,"").replace(/\.\d{3}/,""); }
      ...
      <header style={S.header}>
        <div style={S.headerInner}>
          ...
          <nav style={S.nav}>
            ...
            {teamName&&<>
              <button style={{...S.navBtn,...(view==="pitches"?S.navBtnActive:{})}} onClick={()=>setView("pitches")}>Pitches & Rooms</button>
                ...
                {PITCHES.filter(p=>p.id!=="committee").map((p,i)=>(
                  <div key={p.id} className="pitch-card" style={S.pitchCard}>
                    ...
                    <div style={S.pitchBody}>
                      ...
                      {p.halfable&&<div style={S.halfTag}>⬛ Half-pitch available</div>}
                      <div style={S.pitchActions}>
                        <button style={{...S.bookNowBtn,background:"linear-gradient(135deg,#1e3a8a,#1d4ed8)"}} onClick={()=>{setSelPitch(p.id);setSelHalf("full");setView("book");}}>Book →</button>
                        <button style={S.mapBtn} onClick={()=>window.open(p.mapsUrl,"_blank")}>📍 Map</button>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="fade-in">
            ...
            <section style={S.section}>
              ...
              <div style={{...S.tabRow,marginBottom:8}}>
                <span style={S.tabGroup}>⚽ Pitches</span>
                {PITCHES.filter(p=>p.id!=="committee").map(p=>(<button key={p.id} style={{...S.tab,...(selPitch===p.id?S.tabActive:{})}} onClick={()=>{setSelPitch(p.id);setSelHalf("full");setSelTime(null);}}>{p.icon} {p.name}</button>))}
              </div>
              <div style={S.tabRow}>
                <span style={S.tabGroup}>🪑 Rooms</span>
                <button style={{...S.tab,...(selPitch==="committee"?{...S.tabActive,background:"linear-gradient(135deg,#7c2d12,#ea580c)"}:{})}} onClick={()=>{setSelPitch("committee");setSelHalf("full");setSelTime(null);}}>🪑 Committee Room</button>
              </div>
            </section>
            {selPitch&&pitch?.halfable&&(
              <section style={S.section}>
                <div style={S.sLabel}><span style={S.stepNum}>1b</span> Pitch Section</div>
                <div style={S.tabRow}>{[["full","⬛ Full Pitch"],["north","⬆ North Half"],["south","⬇ South Half"]].map(([v,lbl])=>(<button key={v} style={{...S.tab,...(selHalf===v?S.tabActive:{})}} onClick={()=>{setSelHalf(v);setSelTime(null);}}>{lbl}</button>))}</div>
              </section>
            )}
            <section style={S.section}>
